...
save_path = "data/sampled_{}/APPNP_embeds".format(args.corpus_size)
...
if __name__ == "__main__":
    ...
    model.eval()
    with torch.no_grad():
        Q = model.forward(features, idx_test).cpu().numpy()
        P = labels.cpu().numpy()
    PQ = np.r_[P,Q]
    
    print(PQ.shape, len(Pind), len(Qind))
    embeds = pd.concat([pd.DataFrame(Pind + Qind), pd.DataFrame(PQ)], axis = 1)
    embeds.to_csv(os.path.join(save_path,'APPNP_{}_embeds_{}_{}_{}_{}.txt'.format(args.base, args.delta, args.alpha, args.p, args.seed)), index=False, header=False, sep = ' ')
    print(embeds[:5])
